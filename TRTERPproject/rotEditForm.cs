...
{
    public partial class rotEditForm : Form
    {
        private string docType;
        SqlCommand cmd;
        SqlConnection con = new SqlConnection(ConnectionHelper.ConnectionString);
        public rotEditForm(string docType)
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                string query = "UPDATE BSMGRTRTROT001 SET COMCODE = @COMCODE, DOCTYPE = @DOCTYPE, DOCTYPETEXT = @DOCTYPETEXT, ISPASSIVE = @ISPASSIVE WHERE DOCTYPE = @NEWDOCTYPE";
                cmd = new SqlCommand(query, con);

                cmd.Parameters.AddWithValue("@COMCODE", firmCodeTextBox.Text.Trim());
                cmd.Parameters.AddWithValue("@DOCTYPE", rotTypeTextBox.Text.Trim());
                cmd.Parameters.AddWithValue("@DOCTYPETEXT", rotTypeStatementTextBox.Text.Trim());
        ...
        private void rotEditForm_Load(object sender, EventArgs e)
        {

            using (SqlConnection con = new SqlConnection(ConnectionHelper.ConnectionString))
            {
                string query = "SELECT COMCODE, DOCTYPE, DOCTYPETEXT, ISPASSIVE FROM BSMGRTRTROT001 WHERE DOCTYPE = @DOCTYPE";
                cmd = new SqlCommand(query, con);
                ...
                try
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        rotTypeTextBox.Text = docType;
                        rotTypeStatementTextBox.Text = reader["DOCTYPETEXT"].ToString();
